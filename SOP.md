...
### 4. 長系列專題與摘要優化設計規範
當發布篇數較多（如 5-10 篇以上）的長系列專題時，為維持首頁網格大意摘要的閱讀質感，及各文章頁內系列面板的視覺緊湊度，應遵循以下標準：

1. **避免摘要 (Excerpt) 重複標題與缺乏標點**：
   - 使用自動化處理腳本擷取文章大意（Excerpt）時，**必須跳過封面區與問候語區**（即含有標題與 `嗨，大家好，我是 KC` 的段落）。
   - 摘要應直接從開場白第一段真正的文章本文內容開始，以避免在首頁或文章頁標題下方「重複出現大標且無標點銜接」的閱讀困難現象。

2. **長系列導覽面板（2 欄位響應式網格）**：
   - 針對長系列（如 11 篇的鐵拳教育），內頁系列導覽應避免使用單直欄長列表，需改用 **2 欄位響應式網格** 佈局（Desktop 顯示 2 欄，Mobile 自適應為 1 欄），以縮減佔用的版面高度。
   - 導覽面板需套用該系列專題的專屬 Accent 主色調（例如鐵拳教育專屬的 `#C0512F`），以確保 active 狀態、序號背景及 Hover 動效與系列視覺相稱。
   - 序號排版上，若有系列導讀，可將導讀的序號圓圈文字標記為「**導**」，後續正文則顯示「**1**」至「**N**」，提供直觀的視覺引導。
