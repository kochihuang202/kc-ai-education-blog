...
articles_config = [
    {
        "src_name": "20260723教孩子切割情緒：小怪獸不是你，也不能替你做決定",
    ...
    {
        "src_name": "20260723紫微+AI育兒系列（五）：同一個家，兩張命盤-1",
...
def extract_html_and_metadata(src_dir, config):
    ...
    sections = re.split(r"<(?:section|div)", inner_html)
    actual_text_parts = []
...
def register_post(slug, title, kicker, categories, date, carousel_count):
    ...
    if f'slug: "{slug}"' not in posts_content:
        ...
        related_slugs = ["teach-kids-emotional-separation", "ziwei-ai-parenting-p5", "ziwei-ai-parenting-p4b", "ant-death-spiral-p2"]
...
def main():
    ...
    commit_msg = f"feat: publish two articles (2026-07-23) and deploy {deploy_version}"
