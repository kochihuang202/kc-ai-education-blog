...
src_dir = r"C:\Users\ji3cp\Downloads\20260620記憶學被冤枉·第五篇——象形字圖卡，認字的第一步-1"
...
html_path = os.path.join(src_dir, "記憶學被冤枉·第五篇——象形字圖卡，認字的第一步-1.html")
with open(html_path, "r", encoding="utf-8") as f:
    lines = f.readlines()

# Extract inner HTML (lines 9 to 241, 1-based index)
# lines[8:241]
inner_html = "".join(lines[8:241])

# Placeholder replacement map
card_mapping = [
    "日", "月", "山", "水", "火", "雨", "雲", "泉", "川", "土", "石",
    "口", "目", "耳", "手", "足", "心", "肉", "牙", "齒", "舌", "面", "首"
]

for idx, char in enumerate(card_mapping, 1):
    # Regex to match the placeholder section
    # <section style="background-color:rgba(192,81,47,0.06);padding:40px 20px;margin:0 0 (16|24)px 0;text-align:center;">
    # <p ...>[ 圖卡：char ]</p>
    # </section>
...
for f in os.listdir(src_dir):
    if f.lower().endswith((".png", ".jpg", ".jpeg", ".webp")) and not f.startswith("ChatGPT Image"):
        # Match number prefix: "01_日_圖卡.png"
...
processed_cards = []
for idx, (num, f) in enumerate(card_files, 1):
