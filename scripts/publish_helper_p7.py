...
src_dir = r"C:\Users\ji3cp\Downloads\20260620記憶學被冤枉·第七篇——象形字圖卡（三）：植物草木與器物建築，認字地基打完了-1"
...
html_path = os.path.join(src_dir, "記憶學被冤枉·第七篇——象形字圖卡（三）：植物草木與器物建築，認字地基打完了.html")
with open(html_path, "r", encoding="utf-8") as f:
    lines = f.readlines()

# Extract inner HTML (lines 9 to 221, 1-based index)
# lines[8:221]
inner_html = "".join(lines[8:221])

# Placeholder replacement map (starts at index 45)
card_mapping = [
    "木", "竹", "草（艸）", "禾", "瓜", "果", "麥", "米",
    "門", "戶", "車", "舟", "弓", "刀", "皿", "網", "傘", "冊", "衣", "巾", "絲", "貝", "玉", "瓦", "井", "田", "矢", "盾"
]

for idx, char in enumerate(card_mapping, 45):
...
for f in os.listdir(src_dir):
    if f.lower().endswith((".png", ".jpg", ".jpeg", ".webp")) and not f.startswith("ChatGPT Image"):
        # Match number prefix: "45_木_圖卡.png"
...
processed_cards = []
for idx, (num, f) in enumerate(card_files, 45):
