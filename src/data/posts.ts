...
import lettingKidsExperienceConsequencesWithoutCrueltyHtml from "../article-html/posts/letting-kids-experience-consequences-without-cruelty.html?raw";
import whenKidsSayBoringCutEasyContentHtml from "../article-html/posts/when-kids-say-boring-cut-easy-content.html?raw";
import stopRemindingKidsBuildingRealSelfDisciplineHtml from "../article-html/posts/stop-reminding-kids-building-real-self-discipline.html?raw";
import sighingShakingHeadEmotionalSeparationForKidsHtml from "../article-html/posts/sighing-shaking-head-emotional-separation-for-kids.html?raw";
import givingFreedomWithClearNonNegotiableBoundariesHtml from "../article-html/posts/giving-freedom-with-clear-non-negotiable-boundaries.html?raw";
...
const fearfulFaceInProtectionEducationGraphicBase =
  "https://pub-0eb2a942d02b407091b3e88d3d56fd63.r2.dev/posts/fearful-face-in-protection-education";

const fearfulFaceInProtectionEducationGraphics = Array.from({ length: 8 }, (_, index) => {
  const page = index + 1;
  return {
    src: `${fearfulFaceInProtectionEducationGraphicBase}/graphic-${String(page).padStart(2, "0")}.webp`,
    alt: `最傷孩子的，可能不是你沒教他保護自己，而是你教保護時那張恐懼的臉圖文解析 ${page}/8`
  };
});

const usingAiThinkingPartnerNotAloneGraphicBase =
  "https://pub-0eb2a942d02b407091b3e88d3d56fd63.r2.dev/posts/using-ai-thinking-partner-not-alone";

const usingAiThinkingPartnerNotAloneGraphics = Array.from({ length: 10 }, (_, index) => {
  const page = index + 1;
  return {
    src: `${usingAiThinkingPartnerNotAloneGraphicBase}/graphic-${String(page).padStart(2, "0")}.webp`,
    alt: `我開始用 AI 之後，最大的改變不是少想，而是不再只跟自己想圖文解析 ${page}/10`
  };
});

const readingFlowStateVersusProofOfLearningGraphicBase =
  "https://pub-0eb2a942d02b407091b3e88d3d56fd63.r2.dev/posts/reading-flow-state-versus-proof-of-learning";

const readingFlowStateVersusProofOfLearningGraphics = Array.from({ length: 10 }, (_, index) => {
  const page = index + 1;
  return {
    src: `${readingFlowStateVersusProofOfLearningGraphicBase}/graphic-${String(page).padStart(2, "0")}.webp`,
    alt: `孩子明明讀到停不下來，我卻差點因為一張「閱讀紀錄」打斷她圖文解析 ${page}/10`
  };
});
...
const sighingShakingHeadEmotionalSeparationForKidsGraphicBase =
  "https://pub-0eb2a942d02b407091b3e88d3d56fd63.r2.dev/posts/sighing-shaking-head-emotional-separation-for-kids";

const sighingShakingHeadEmotionalSeparationForKidsGraphics = Array.from({ length: 10 }, (_, index) => {
  const page = index + 1;
  return {
    src: `${sighingShakingHeadEmotionalSeparationForKidsGraphicBase}/graphic-${String(page).padStart(2, "0")}.webp`,
    alt: `孩子一被提醒就嘆氣、甩頭、走人，我以前只看見「沒禮貌」圖文解析 ${page}/10`
  };
});

const givingFreedomWithClearNonNegotiableBoundariesGraphicBase =
  "https://pub-0eb2a942d02b407091b3e88d3d56fd63.r2.dev/posts/giving-freedom-with-clear-non-negotiable-boundaries";

const givingFreedomWithClearNonNegotiableBoundariesGraphics = Array.from({ length: 8 }, (_, index) => {
  const page = index + 1;
  return {
...
export const posts = [
  ...
  },
  {
    ...
    kicker: "KC 育兒手記",
    excerpt:
      "你教孩子保護身體、認識界線、說不、跑、求救，這些都是對的。但你有沒有想過，你教這些的時候，臉上的表情與緊張，本身就在傳遞「這件事會毀了你」。真正的傷害是身體上的，但文化疊加的羞恥感，才是我們最該替孩子拆掉的二次傷害。",
    categories: ["parents", "core"],
    coverImage: fearfulFaceInProtectionEducationGraphics[0].src,
    coverAlt: fearfulFaceInProtectionEducationGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: fearfulFaceInProtectionEducationGraphics
    },
    relatedPosts: [],
    body: fearfulFaceInProtectionEducationHtml
  },
  {
    ...
    kicker: "自學省思 · 第7篇",
    excerpt:
      "開始自學之後，最困擾我的往往不是選擇教材，而是沒有標準答案的微小育兒問題：孩子拖延、喊無聊、自律靠提醒、情緒反應強。好好使用 AI，不是為了偷懶少想，而是找一個強大的對手，不再讓自己一個人閉門造車。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: usingAiThinkingPartnerNotAloneGraphics[0].src,
    coverAlt: usingAiThinkingPartnerNotAloneGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
  ...
  },
  {
    ...
    kicker: "自學省思 · 第4篇",
    excerpt:
      "女兒跟我說：「我一看書就會陷進去，所以我不想做紀錄。」大人常因為自己的不安與焦慮，急著要求孩子給出證明與成果，卻差點用功利主義打斷了孩子最珍貴、最難得的「心流狀態」。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: readingFlowStateVersusProofOfLearningGraphics[0].src,
    coverAlt: readingFlowStateVersusProofOfLearningGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: readingFlowStateVersusProofOfLearningGraphics
    },
    relatedPosts: ["when-kids-say-boring-cut-easy-content", "sighing-shaking-head-emotional-separation-for-kids", "stop-reminding-kids-building-real-self-discipline"],
    body: readingFlowStateVersusProofOfLearningHtml
  },
  {
    ...
    kicker: "自學省思 · 第6篇",
    excerpt:
      "孩子拖延到半夜才要做任務，我曾想狠下心「陪她做到凌晨兩點，用痛苦讓她記住教訓」。但我突然發覺：強迫折磨只是大人內心憤怒的宣洩，並不能培養真正的責任感。界線與後果，不等於殘忍地看著孩子崩潰。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: lettingKidsExperienceConsequencesWithoutCrueltyGraphics[0].src,
    coverAlt: lettingKidsExperienceConsequencesWithoutCrueltyGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: lettingKidsExperienceConsequencesWithoutCrueltyGraphics
    },
    relatedPosts: ["sighing-shaking-head-emotional-separation-for-kids", "using-ai-thinking-partner-not-alone", "giving-freedom-with-clear-non-negotiable-boundaries"],
    body: lettingKidsExperienceConsequencesWithoutCrueltyHtml
  },
  {
    ...
    relatedPosts: ["stop-reminding-kids-building-real-self-discipline", "reading-flow-state-versus-proof-of-learning", "using-ai-thinking-partner-not-alone"],
  ...
  },
  {
    ...
    kicker: "自學省思 · 第2篇",
    excerpt:
      "我給孩子排了每日任務清單，以為這就是自主。直到我發覺每天真正出現的畫面，是我在後面狂盯時間、倒數提醒。你以為在教自律，其實只是你在幫她自律。當我停止提醒，孩子才真正開始掌握時間。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: stopRemindingKidsBuildingRealSelfDisciplineGraphics[0].src,
    coverAlt: stopRemindingKidsBuildingRealSelfDisciplineGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: stopRemindingKidsBuildingRealSelfDisciplineGraphics
    },
    relatedPosts: ["giving-freedom-with-clear-non-negotiable-boundaries", "when-kids-say-boring-cut-easy-content", "letting-kids-experience-consequences-without-cruelty"],
    body: stopRemindingKidsBuildingRealSelfDisciplineHtml
  },
  {
    ...
    kicker: "自學省思 · 第5篇",
    excerpt:
      "孩子被提醒就嘆氣、甩頭甚至轉身走開，大人第一時間只看見「沒禮貌」想立刻糾正。但比講禮貌更重要的是引導孩子了解：她不是故意忤逆，而是不知道怎麼處理情緒。允許生氣，但教孩子不把主導權交給情緒小怪獸。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: sighingShakingHeadEmotionalSeparationForKidsGraphics[0].src,
    coverAlt: sighingShakingHeadEmotionalSeparationForKidsGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: sighingShakingHeadEmotionalSeparationForKidsGraphics
    },
    relatedPosts: ["reading-flow-state-versus-proof-of-learning", "letting-kids-experience-consequences-without-cruelty", "giving-freedom-with-clear-non-negotiable-boundaries"],
    body: sighingShakingHeadEmotionalSeparationForKidsHtml
  },
  {
    ...
    kicker: "自學省思 · 第1篇",
    excerpt:
      "給了孩子很大的自由後，她開始發現原來討厭的事可以直接說「不要」，甚至進入僵局。大人意識到：給自由不等於無限退讓。沒有界線的自由會變成無序，適度收回部分決定權、守住底線，才是安全的自由。",
    categories: ["self-learning-reflection", "parents", "core"],
    coverImage: givingFreedomWithClearNonNegotiableBoundariesGraphics[0].src,
    coverAlt: givingFreedomWithClearNonNegotiableBoundariesGraphics[0].alt,
    gallery: {
      label: "<圖文解析>",
      images: givingFreedomWithClearNonNegotiableBoundariesGraphics
    },
    relatedPosts: ["stop-reminding-kids-building-real-self-discipline", "letting-kids-experience-consequences-without-cruelty", "using-ai-thinking-partner-not-alone"],
    body: givingFreedomWithClearNonNegotiableBoundariesHtml
  },
